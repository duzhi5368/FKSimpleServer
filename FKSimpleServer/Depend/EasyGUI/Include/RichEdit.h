...
public:
	virtual BOOL	Create(HWND hParents, DWORD dwStyle, int x, int y, int width, int height, TCHAR * pText);
	virtual BOOL	printf( DWORD dwColor, TCHAR * pBuff, ... );
	virtual BOOL	AddLine(DWORD dwColor, TCHAR * pLine);
	...
	virtual BOOL	GetCharRange( CHARRANGE &chrge, TCHAR * pRetString )
	{
		TEXTRANGE tr;
		tr.chrg = chrge;
